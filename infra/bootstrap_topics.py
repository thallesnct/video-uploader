        ...
        out.append(
            {
                ...
                "replication_factor": rf,
                "configs": {"retention.ms": str(90 * 24 * 3600 * 1000),
                            "min.insync.replicas": str(min_isr)},
            }
        )
    return out
...
def main() -> int:
    ...
    existing = {line.strip() for line in kafka(
        "kafka-topics", "--bootstrap-server", BROKER, "--list").splitlines()
    ...
    for topic in planned:
        ...
        args = [
            ...
            "--if-not-exists", "--topic", topic["name"],
    ...
    for line in describe.splitlines():
        if line.startswith("Topic:") and "PartitionCount:" in line:
            parts = dict(
                p.split(":", 1) for p in
                (chunk.strip() for chunk in line.split("\t")) if ":" in p
            )
    ...
    print(f"topics: {len(planned)} declared, {created} created, "
    ...
    if drift:
        ...
        print("Partitions can be increased but never decreased; to fix a topic "
              "that is too large, delete and recreate it.", file=sys.stderr)
