...
2. A DB claim — mutual exclusion against a message being processed twice
   *concurrently* (a manual DLQ replay racing a live retry-tier message for the
   ...
   prevent. See `RenditionRepository.claim`'s docstring.
"""
...
from pipeline.storage import ObjectStore, object_store, scratch_key
...
def build_handler(
    ...
    `transcode_fn` is injected so the Kafka round trip, the idempotency skip,
    and the claim can all be tested on a machine without ffmpeg (ADR-0011); the
    real ffmpeg path is covered separately by tests marked `ffmpeg`.
    """
    ...
    def handle(event: Event, view: MessageView) -> None:
        ...
        existing = store.head(event.target_key)
        if existing is not None:
            log.info(
                "rendition already present for video=%s rendition=%s — "
                "re-announcing without re-encoding",
                event.video_id,
                event.rendition,
            )
            _announce(producer, event, int(existing.get("ContentLength", 0)), 0.0)
            return
        ...
        if not claimed:
            raise TransientError(
                f"another attempt already holds the claim for {event.video_id}/{event.rendition}"
            )
        ...
        with tempfile.TemporaryDirectory(dir=scratch_dir) as scratch:
            ...
            remote_scratch = scratch_key(event.owner_id, event.video_id, f"{event.rendition}.part")
        ...
        _announce(producer, event, size_bytes, elapsed)
